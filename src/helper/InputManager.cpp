...
InputManager* InputManager::Instance() {
  ...
  if (sInstance == NULL) sInstance = new InputManager();
...
InputManager::InputManager() {
  ...
  // Copying the contents of mKeyboardState into mPrevKeyboardState so that it
  // contains data on the first frame
...
bool InputManager::KeyPressed(SDL_Scancode scanCode) {
  // returning true of the key was not pressed in the previous keyboard state
  // but is pressed in the current one
  return (mPrevKeyboardState[scanCode]) == 0 && (mKeyboardState[scanCode] != 0);
}

bool InputManager::KeyReleased(SDL_Scancode scanCode) {
  // returning true of the key was pressed in the previous keyboard state but is
  // not pressed in the current one
  return (mPrevKeyboardState[scanCode] != 0) && (mKeyboardState[scanCode] == 0);
}

Vector2 InputManager::MousePos() {
  // Converting the mouse position into a Vector2 for ease of use since all
  // entities use Vector2
...
bool InputManager::MouseButtonPressed(MOUSE_BUTTON button) {
  ...
  // return true if the mask does not exist in the previous mouse state, but
  // exists in the current one
...
bool InputManager::MouseButtonReleased(MOUSE_BUTTON button) {
  ...
  // return true if the mask exists in the previous mouse state, but does not
...
void InputManager::UpdatePrevInput() {
  // Copying the keyboard state into the previous keyboard state array at the
  // end of the frame
  memcpy(mPrevKeyboardState, mKeyboardState, mKeyLength);
  // Setting the previous mouse state as the current mouse state at the end of
  // the frame
