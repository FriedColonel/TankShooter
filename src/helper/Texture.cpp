...
Texture::Texture(std::string filename) {
  ...
  // Loads the texture from the AssetManager to avoid loading textures more than
  // once
...
Texture::Texture(std::string filename, int x, int y, int w, int h) {
  ...
  // Setting the clipped rectangle to only get the needed texture from the
  // spritesheet
  mClipRect.x = x;
  mClipRect.y = y;
  mClipRect.w = mWidth;
...
void Texture::Render() {
  ...
  // Centers the texture on the texture's world position so that its position is
