...
// By: Ather Omar                                                      //
//---------------------------------------------------------------------//
#include <QuickSDL/AssetManager.h>
//-----------------------------------------------------------------
// QuickSDL
//-----------------------------------------------------------------
namespace QuickSDL {
...
SDL_Texture* AssetManager::GetTexture(std::string filename) {
  ...
  // If the file has not been already loaded, load it and add it to the
  // mTextures map
...
TTF_Font* AssetManager::GetFont(std::string filename, int size) {
  ...
  // The key takes into account the size of the font aswell since the same font
  // can be opened with different sizes
  std::string key = fullPath + (char)size;

  // If the font has not been already loaded, load it and add it to the mFonts
  // map
  if (mFonts[key] == nullptr) {
    mFonts[key] = TTF_OpenFont(fullPath.c_str(), size);
    // Error handling for opening the font
    if (mFonts[key] == nullptr)
      printf("Font Loading Error: Font-%s Error-%s", filename.c_str(),
             TTF_GetError());
  }
...
SDL_Texture* AssetManager::GetText(std::string text, std::string filename,
  ...
  // The key takes into account the font, text, size, and color to differentiate
  // text textures
  std::string key = text + filename + (char)size + (char)color.r +
                    (char)color.b + (char)color.g;

  // If the same text has not been rendered before, render it and add it to the
  // mText map
...
Mix_Music* AssetManager::GetMusic(std::string filename) {
  ...
  // If the file has not been already loaded, load it and add it to the mMusic
  // map
