...
int menu::display() {
  ...
  while (true) {
    ...
    if (previous < 0) previous = num - 1;
    if (previous == num) previous = 0;
    int temp =
        (entries[previous].desc.length() - entries[selected].desc.length());
    for (int i = 0; i < temp; i++) cout << " ";
    cout << '\n';
    opt = getch();
    if (opt == 10) return entries[selected].ret;
    switch (opt) {
      case 66:
        selected++;
        break;
...
void menumaker() {
  ...
  cout
      << "Please copy code from file temp and place in the appropriate place\n";
  outf.close();
}
